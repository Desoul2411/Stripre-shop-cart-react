...
async function createCheckoutSession(req,res) {
  try {
    const cartItems = req.body; // req.body is already parsed, because we used app.use(express.json()) in index.js
   
    const line_items = validateCartItems(products, cartItems);  //products - inventory to compare with cartItems in order to provide that valid product data was sent from client
...
async function getCheckoutSession(req, res) {
  const { sessionId } = req.params;  // router.get('/checkout-sessions/:sessionId', getCheckoutSession);   //  in Result.js  axios(`api/checkout-sessions/${sessionId}`  
